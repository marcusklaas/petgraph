...
use {Directed, Direction, EdgeType, Graph, Incoming, Outgoing, Undirected};
...
use visit::{
    ...
    NodeIndexable, Visitable, VisitMap, IntoNodeIdentifiers,
};
use IntoWeightedEdge;

// reexport those things that are shared with Graph
#[doc(no_inline)]
pub use graph::{edge_index, node_index, DefaultIx, EdgeIndex, GraphIndex, IndexType, NodeIndex};
    ...
    Ix: IndexType,
{
        ...
        K: Default + Copy + PartialOrd + Sub<K, Output = K> + Add<K, Output = K>,
    {
        ...
        arc_set.sort_by(|a, b| a.3.partial_cmp(&b.3).unwrap_or(cmp::Ordering::Equal));
